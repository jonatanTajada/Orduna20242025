...
public class ExploradorDeArchivos extends JFrame {
	...
	public ExploradorDeArchivos() {
		...
		arbolDirectorios.addTreeSelectionListener(new TreeSelectionListener() {
			...
			public void valueChanged(TreeSelectionEvent e) {
				DefaultMutableTreeNode nodoSeleccionado = (DefaultMutableTreeNode) arbolDirectorios
		...
		tablaArchivos.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (SwingUtilities.isRightMouseButton(e) && tablaArchivos.getSelectedRow() != -1) {
					tablaArchivos.setRowSelectionInterval(tablaArchivos.rowAtPoint(e.getPoint()),
							tablaArchivos.rowAtPoint(e.getPoint())); // Selecciona la fila
		...
		JSplitPane panelDividido = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(arbolDirectorios),
				new JScrollPane(tablaArchivos));
		panelDividido.setDividerLocation(250);

		// Añadir componentes a la ventana
		add(panelDividido, BorderLayout.CENTER);
		add(new JButton("Actualizar"), BorderLayout.SOUTH); // Botón de actualizar en la parte inferior
	...
	private void eliminarArchivo() {
		...
		if (filaSeleccionada != -1) {
			...
			int confirmacion = JOptionPane.showConfirmDialog(this, "¿Está seguro de que desea eliminar este archivo?",
					"Confirmar Eliminación", JOptionPane.YES_NO_OPTION);
			if (confirmacion == JOptionPane.YES_OPTION) {
				if (archivo.delete()) {
					cargarArchivosEnTabla(archivo.getParentFile());
				} else {
					JOptionPane.showMessageDialog(this, "No se pudo eliminar el archivo.", "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		}
